...
const IMAGE_BY_NAME = {
  ...
  'Ściąganie uchwytu neutralnego do klatki':'eximg/neutral_pulldown_fixed.png',
...
const EXERCISE_LIBRARY = [
  ...
  {name:'Ściąganie uchwytu neutralnego do klatki', muscle:'Plecy', equipment:'Wyciąg', category:'pulldown', imageSrc:'eximg/neutral_pulldown_fixed.png', sets:3,min:8,max:12,step:5,note:'Bez bujania.'},
...
function migratePlanData(){
  ...
  data.plan.B = data.plan.B.filter(ex => ex.id !== 'B8' && ex.name !== 'Chest press na maszynie — chwyt neutralny');
  data.plan.B = data.plan.B.filter(ex => ex.category !== 'legcurl' && !/^Uginanie nóg/i.test(ex.name));
  let seenB4=false;
  data.plan.B = data.plan.B.filter(ex => {
    if(ex.id==='B4' || ex.name==='Prostowanie nóg na maszynie — obie nogi'){
      if(seenB4) return false; seenB4=true;
    }
    return true;
  });
